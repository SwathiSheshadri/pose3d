...
switch whichfilter
    case 1 %moving average
        for i = 1:3*nfeatures
            ...
            coords3dbp(:,i) = conv(coords3dbp(:,i),1/npoints*ones(1,npoints),'same');
    ...
    case 2 %median filter
        for i = 1:3*nfeatures
            ...
            coords3dbp(:,i) = oneD_medfilt(coords3dbp(:,i),npoints);
...
for ipairs = 1:size(drawline,1)
    ...
    reconall(:,ipairs) = sqrt(sum((coords3dall(:,3*(i_1-1)+1:3*i_1) - coords3dall(:,3*(i_2-1)+1:3*i_2)) .^ 2,2));
    reconbp(:,ipairs) = sqrt(sum((coords3dbp(:,3*(i_1-1)+1:3*i_1) - coords3dbp(:,3*(i_2-1)+1:3*i_2)) .^ 2,2));
    reconavg(:,ipairs) = sqrt(sum((coords3davg(:,3*(i_1-1)+1:3*i_1) - coords3davg(:,3*(i_2-1)+1:3*i_2)) .^ 2,2));
...
for i =1:npoints:size(coords3dall,1) %movies files saved as .mat file for every 5th point (to make trying out demo easier)
